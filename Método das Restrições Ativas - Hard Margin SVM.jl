...
function separar_treino_teste(dados, porc_treino=0.8)

    # Separa a matriz com o conjunto de dados inteiro em dois conjuntos: treino e teste
    # dados é a matriz e porc_treino é a porcentagem dos dados que irão compor o conjunto de treino

    num_pontos_total = Int(size(dados, 1))
    num_pontos_treino = Int(round(num_pontos_total * porc_treino))
    num_pontos_teste = num_pontos_total - num_pontos_treino

    indices_conj_treino = shuffle(1:num_pontos_total)[1:num_pontos_treino]
    indices_conj_teste = setdiff(1:num_pontos_total, indices_conj_treino)

    conjunto_de_treino = dados[indices_conj_treino, :]
    conjunto_de_teste = dados[indices_conj_teste, :]
...
function matriz_de_confusao(x_estrela, conj_teste)

    # Retorna a quantidade de verdadeiros positivos, verdadeiros negativos, falsos positivos e falsos negativos (em forma de string).
    # x_estrela é um vetor cujas coordenadas correspondem a uma reta e conj_teste é o conjunto de dados a serem testados.
    # Por exemplo, se x_estrela = [w_1, w_2, b], então a reta utilizada na classificação é w_1 x_1 + w_2 x_2 + b = 0.
    ...
    num_pontos = Int(size(conj_teste, 1))  # Número de pontos pertencentes ao conjunto de teste

    for i = 1:num_pontos

        ponto = conj_teste[i, 1:end-1]    # Cria o ponto na forma [x_1, x_2] do conjunto de dados para a iteração
        push!(ponto, 1)                   # Transforma a lista na forma: ponto = [x_1, x_2, 1]

        if conj_teste[i, end] == 1.0        # Ponto é da classe positiva

            if x_estrela' * ponto > 0         # Ponto é da classe positiva e classificado corretamente
                
                ver_pos += 1

            elseif x_estrela' * ponto < 0     # Ponto é da classe positiva, mas classificado erroneamente

                fal_neg += 1

            end

        elseif conj_teste[i, end] == -1.0    # Ponto é da classe negativa

            if x_estrela' * ponto > 0         # Ponto é da classe negativa, mas classificado erroneamente
                
                fal_pos += 1

            elseif x_estrela' * ponto < 0     # Ponto é da classe negativa e classificado corretamente
    ...
    # Há a possibilidade de x_estrela ser o valor oposto do que esperamos. Quando isso ocorre, o semiespaço positivo de x_estrela conterá os pontos da classe negativa.
    # Assim sendo, verificamos se a maioria dos pontos foi classificada corretamente. Se não, os números que obtivemos precisam ser trocados.

    num_acertos = ver_pos + ver_neg
    num_erros = fal_pos + fal_neg

    if num_erros > num_acertos

        verdadeiros_positivos = fal_neg
        falsos_negativos = ver_pos
        falsos_positivos = ver_neg
        verdadeiros_negativos = fal_pos

    else

        verdadeiros_positivos = ver_pos
        falsos_negativos = fal_neg
        falsos_positivos = fal_pos
        verdadeiros_negativos = ver_neg

    end

    println("Matriz de confusão obtida:")
    println("    VP = $(verdadeiros_positivos),    FN = $(falsos_negativos)")
    println("    FP = $(falsos_positivos),    VN = $(verdadeiros_negativos)")
...
# Separar dados do conjunto em treino e teste:
# Obs: esse passo é opcional e, para verificar a acurácia da classificação, utilize a função matriz_de_confusao.

A, conj_teste = separar_treino_teste(A)    # Algumas linhas de A são retiradas e guardadas na variável conj_teste (conjunto de teste).
...
# Acurácia do classificador (obs: utilizar esse comando apenas se a função separar_treino_teste foi usada):
println(matriz_de_confusao(x, conj_teste))
